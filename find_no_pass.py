...
'''read marks of many workbooks then write the marks togather for

students which failed pass the exam.
...
from getdir import getdir, getselefile     # 2017-3-22
...
# logging.basicConfig(filename='loglearn.txt', level=logging.DEBUG,
...
for file in os.listdir(dirname):
    ...
    if classReg.search(file):
        ...
        for row in range(3, sheet.max_row+1):
            ...
            logging.info(str(sheet['J'+str(row)].value))    # 总成绩在J列
            if sheet['J'+str(row)].value:
                if(int(sheet['J'+str(row)].value) < 59):
                    # Find the mark that no passed.
                    logging.debug('课程'+sheet['B'+str(row)].value)
                    # 课程在B列
                    logging.debug('学号'+sheet['C'+str(row)].value)
                    # 学号在C列
                    logging.debug('姓名'+sheet['D'+str(row)].value)
                    # 姓名在D列
                    logging.debug(' 课堂平时成绩：'+str(sheet['E'+str(row)].value))
                    # 课堂平时成绩在E列
                    logging.debug(' 课堂期末成绩：'+str(sheet['F'+str(row)].value))
                    # 课堂期末成绩在F列
                    logging.debug(' 课堂总成绩：'+str(sheet['G'+str(row)].value))
                    # 课堂总成绩在G列
                    logging.debug(' 实践成绩：'+str(sheet['H'+str(row)].value))
                    # 实践成绩在H列
                    logging.debug(' 实验成绩：'+str(sheet['I'+str(row)].value))
                    # 实验成绩在I列
                    logging.debug(' 总成绩：'+str(sheet['J'+str(row)].value))
                    # 总成绩在J列
